...
struct AppTests {
  ...
  func isUsernameExists() async throws {
    try await withApp { app in
      try await app.testing().test(
        .GET, "/api/auth/username-exists?username=\(username1)",
        afterResponse: { res in
          #expect(res.status == .ok)
        })
      try await app.testing().test(
        .GET, "/api/auth/username-exists?username=\(nonexistentUsername)",
        afterResponse: { res in
          #expect(res.status == .ok)
        })
    }
  ...
  func addDiscussion() async throws {
    try await withApp { app in
      try await app.testing().test(
        .POST, "/api/discussions/create/test",
        beforeRequest: { req in
          print(authToken ?? "TOKEN NOT FOUND")
          req.headers.bearerAuthorization = .init(token: authToken!)
        },
  ...
  func addComment() async throws {
    try await withApp { app in
      try await app.testing().test(
        .POST,
        "/api/discussions/\(discussionId ?? "DISCUSSION ID NOT FOUND")/comments/add?content=test",
        beforeRequest: { req in
          print(authToken ?? "TOKEN NOT FOUND")
          req.headers.bearerAuthorization = .init(token: authToken!)
        },
  ...
  func getComments() async throws {
    try await withApp { app in
      try await app.testing().test(
        .GET, "/api/discussions/\(discussionId ?? "DISCUSSION ID NOT FOUND")/details",
        beforeRequest: { req in
          print(authToken ?? "TOKEN NOT FOUND")
          req.headers.bearerAuthorization = .init(token: authToken!)
        },
  ...
  func deleteComment() async throws {
    try await withApp { app in
      try await app.testing().test(
        .DELETE,
        "/api/discussions/\(discussionId ?? "DISCUSSION ID NOT FOUND")/comments/delete/\(commentId ?? "COMMENT ID NOT FOUND")",
        beforeRequest: { req in
          print(authToken ?? "TOKEN NOT FOUND")
          req.headers.bearerAuthorization = .init(token: authToken!)
        },
  ...
  func leaveDiscussion() async throws {
    try await withApp { app in
      try await app.testing().test(
        .DELETE, "/api/discussions/\(discussionId ?? "DISCUSSION ID NOT FOUND")/leave",
        beforeRequest: { req in
          print(authToken ?? "TOKEN NOT FOUND")
          req.headers.bearerAuthorization = .init(token: authToken!)
        },
  ...
  func deleteDiscussion() async throws {
    try await withApp { app in
      try await app.testing().test(
        .DELETE, "/api/discussions/\(discussionId ?? "DISCUSSION ID NOT FOUND")/delete",
        beforeRequest: { req in
          print(authToken ?? "TOKEN NOT FOUND")
          req.headers.bearerAuthorization = .init(token: authToken!)
        },
