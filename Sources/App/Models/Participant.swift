...
final class Participant: Model, Content, @unchecked Sendable {
  ...
  init(
    id: UUID? = nil, discussionId: Discussion.IDValue, userId: User.IDValue, joinedAt: Date? = nil,
    status: Status = .inactive, lastActiveAt: Date? = nil, isAuthor: Bool = false
  ) {
    self.id = id
...
extension Participant {
  static func updateLastActive(_ participantId: UUID, on: any Database) async throws {
    try await Participant.query(on: on)
      .filter(\.$id == participantId)
      .set(\.$lastActiveAt, to: Date())
      .update()
  }

  static func updateStatus(_ participantId: UUID, status: Status, on: any Database) async throws {
    try await Participant.query(on: on)
      .filter(\.$id == participantId)
